...
- **Not** Google Labs typography fidelity. Fixtures may match the mirror exactly
  because both fall back to the same stack — that is _controlled reference-fixture
  parity_, and the reports say so in those words.
...
and logos never cross into `src/`. What crosses the boundary is _structural
design intelligence_ — spacing rhythm, container logic, shape scales, motion
