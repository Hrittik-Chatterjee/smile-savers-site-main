...
## DDR-019 — Full 9-viewport x 32-route overflow sweep found 3 real bugs

**Status:** Accepted

**Evidence (FACT).** Extended the earlier 320/768/1440 spot-check (Part 24) to
the full 320/360/375/390/414/768/1024/1280/1440 x 32-route matrix (288
combinations). Found 34 failing combinations across 5 routes, reducing to 3
distinct root causes:

1. **CSS Grid `min-width: auto` sizing bug in `LocalHero.astro`.** `.hero-container`
   is `display: grid`; grid items default to `min-width: auto`, so its single
   mobile column sized to `.hero-content`'s max-content width (measured
   638px) instead of the 320px viewport, on every viewport below 1024px.
   Fixed with the standard remedy: `min-width: 0` on the grid item. The
   second column (`.hero-map`, active at 1024px+) had the identical bug,
   measured 1213px on a 1024px viewport — same fix applied.

2. **Same bug, one level up, in `RelatedServices.astro`.** `.service-info`
   (the inner flex child) already had `min-width: 0` for its own shrink, but
   `.service-link-card` — the outer grid item in `.services-grid` — did not,
   so the grid track still sized to the card's own max-content width. Fixed
   the same way. This single fix cleared `/compare/implants-vs-dentures/`,
   `/for/families/`, and `/learn/dental-implants/` entirely.

3. **A genuine class-name collision with a reserved DaisyUI component class**,
   found only after the grid fixes reduced but did not eliminate overflow on
   the two location-detail routes. `LocalHero.astro` used `.hero-content` —
   DaisyUI ships its own global (unscoped) `.hero-content` utility as part of
   its documented `hero` component
   (`isolation:isolate; display:flex; max-width:80rem; padding:1rem; ...`),
   and it applied *alongside* the page's own scoped rule rather than being
   overridden by it, since the two rules touched different properties. This
   is exactly the naming-collision risk `naming-conventions.md` flagged in
   the abstract (siblings sharing a stem) materialising for real, against a
   third-party library rather than another component in this repo. Renamed
   to `.local-hero-content`, confirmed no other `.hero-*` class in the same
   file collides with a DaisyUI reserved name.

**Method note.** Root-caused each failure by direct browser measurement
(`getBoundingClientRect`, `getComputedStyle`, and — for the collision — a
grep of the actual *built* CSS bundle, which is what surfaced the DaisyUI
rule; grepping only `src/` found nothing because the colliding rule is
generated by the DaisyUI plugin, not hand-authored). A naive full-route
Playwright loop at 9 viewports x 32 routes with `networkidle` timed out the
tool's execution window; switched to `domcontentloaded` (layout is what's
being measured, not full asset load) and ran as a background job.
